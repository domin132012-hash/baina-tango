...
const SITE_URL = process.env.SITE_URL || "https://venerable-pavlova-6bcd56.netlify.app";
  ...
  try {
    ...
    if (!token) {
      return {
        statusCode: 401,
        headers,
        body: JSON.stringify({ error: "请先登录账号后再购买。" })
      };
    }
    ...
    if (userError || !userData || !userData.user) {
      return {
        statusCode: 401,
        headers,
        body: JSON.stringify({ error: "登录状态无效，请重新登录。" })
      };
    }
    ...
    if (!product) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: "商品不存在。" })
      };
    }
