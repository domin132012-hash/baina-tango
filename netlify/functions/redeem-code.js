...
const DEVELOPER_REDEEM_CODE = process.env.DEVELOPER_REDEEM_CODE;
...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (!DEVELOPER_REDEEM_CODE) {
      return json(500, {
        error: "开发者兑换码环境变量未设置：DEVELOPER_REDEEM_CODE"
      });
    }
    ...
    const devCode = normalizeCode(DEVELOPER_REDEEM_CODE);

    if (!inputCode) {
      return json(400, { error: "请输入兑换码" });
    }

    if (inputCode !== devCode) {
      return json(400, { error: "兑换码无效" });
    }
    ...
    const oldWordBonus = Number(
      oldEntitlement && oldEntitlement.word_limit_bonus
        ? oldEntitlement.word_limit_bonus
        : 0
    );

    const oldAiQuota = Number(
      oldEntitlement && oldEntitlement.ai_quota
        ? oldEntitlement.ai_quota
        : 0
    );
    ...
    if (upsertError) {
      ...
      return json(500, {
        error: "写入开发者权益失败：" + upsertError.message
      });
    }
    ...
    return json(500, {
      error: err && err.message ? err.message : "兑换失败"
    });
  }
};
