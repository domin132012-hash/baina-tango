...
// 这里必须填支持图片输入的豆包视觉 / 多模态模型 ID
const DOUBAO_VISION_MODEL =
  process.env.DOUBAO_VISION_MODEL || process.env.DOUBAO_MODEL;
...
function cleanDataUrl(s) {
  ...
  if (!x.startsWith("data:image/")) {
    throw new Error("图片格式错误，请上传 JPG / PNG / WebP");
  }
  if (x.length > 5.8 * 1024 * 1024) {
    throw new Error("图片太大，请裁切或压缩后再上传");
  }
  return x;
}
...
function normalizeItem(x) {
  ...
  const reading = String(x.reading || x.kana || "").trim();
  const definition = String(x.definition || x.meaning || x.zh || "").trim();
  const pos = String(x.pos || "词语").trim();

  if (!word || !definition) return null;

  return {
    word: word.slice(0, 40),
    reading: reading.slice(0, 30),
    pos: pos.slice(0, 12),
...
async function callDoubaoVision(imageDataUrl, maxWords) {
  if (!DOUBAO_API_KEY) {
    throw new Error("豆包 API Key 未设置");
  }

  if (!DOUBAO_VISION_MODEL) {
    throw new Error("豆包视觉模型 ID 未设置：请添加 DOUBAO_VISION_MODEL");
  }
      ...
      messages: [
        {
          role: "system",
          content:
            "你是日语学习图片识别助手。必须只返回合法 JSON，不要 Markdown。"
        },
        {
          role: "user",
          content: [
  ...
  if (!res.ok) {
    console.error("Doubao image error:", data);
    const msg =
...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (entitlementError) {
      return json(500, {
        error: "读取 AI 次数失败：" + entitlementError.message
      });
    }

    const aiQuota = Number(entitlement && entitlement.ai_quota ? entitlement.ai_quota : 0);

    if (aiQuota <= 0) {
      return json(402, {
        error: "AI 次数不足，请先购买 AI 次数包"
      });
    }

    const aiResult = await callDoubaoVision(imageDataUrl, maxWords);
    ...
    if (updateError) {
      return json(500, {
        error: "扣除 AI 次数失败：" + updateError.message
      });
    }
    ...
    return json(500, {
      error: err && err.message ? err.message : "图片识别失败"
    });
  }
};
