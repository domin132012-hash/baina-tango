...
> 最后更新：2026-06-24。配套阅读：`AGENTS.md`（开工规则）、`PROJECT_STATUS.md`（当前进度）、`AGENT_SYNC_BOARD.md`（实时同步看板）、`AGENT_WORKLOG.md`（最近动作流水）、`NOTICES_ADMIN.md`（通知后台）。
...
- 综合科目：2024 一套 MVP 已完成并上线。
- EJU 記述作文：PR #2 已在用户完成真实 Cloudflare Branch Preview 验收后合并到 `main`，Production 已部署 active。入口为 `学习 → 真题试炼 → 日本語 → 記述`。
- 词典优先查词：架构计划在 `docs/architecture/DICTIONARY_LOOKUP_PLAN.md`。Production 当前使用 R2 shard lookup（active version `jmdict-zh-deepseek-top1000-reviewed-r2-20260624`），中文 overlay 已对 Top 1000 reviewed-r2 词条生效。原有 English-only R2 shards（`jmdict-english-r2-shards-2026-06-18`）保留为回滚点。命中词典不默认调用 AI（`aiCalled=false`），未命中只提示可尝试 AI 解释。
- 完整 JMdict R2 sharded lookup：PR #6 已 merge。D1 full import 仍禁止。
- 中文释义 overlay：Top 1000 reviewed-r2 batch（500 entries / 799 senses）已在 production 激活，version `jmdict-zh-deepseek-top1000-reviewed-r2-20260624`。R2 写入 513 objects，D1 写入 5 rows。Codex 完成部署后被 usage limit 中断；Hermes 完成 closeout（docs + commit + push）。PR #12 保持 draft/open/unmerged。
- 代理 closeout 机制：`docs/ops/AGENT_CLOSEOUT_CHECKLIST.md` 是非平凡任务收尾必读文件。
