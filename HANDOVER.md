...
- 登录后主界面五栏导航：Issue #11 的 UI 专用分支是 `feat/post-login-nav-restructure`，从 `main` 的 `ebc320317e6ef212a38a53a603191c419aca527c` 创建，不能在词典 overlay 分支 `feat/dictionary-zh-deepseek-pilot-100` 上继续。实现 commit `1f0759015a701c38c20f0bca8a38e02870b07abd` 将底部导航改为 `学习 / 词库 / 首页 / 社区 / 我的`；`学习` 包含 EJU/JLPT/口语，`词库` 集中查词/背词/导入导出/词库管理，`社区` 只放建设中入口，`我的` 增加邀请制度和给开发者建议占位。未上线功能统一显示 `建设中` 并只触发 toast。此任务不得部署 Preview/Production、不得调用 DeepSeek/Google Translate/外部 AI、不得写 R2/D1、不得修改 Cloudflare 配置、不得触碰 PR #12 词典 overlay artifacts。
