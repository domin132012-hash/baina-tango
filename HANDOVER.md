...
- DeepSeek 中文释义 overlay pilot：Issue #11 / PR #12 / branch `feat/dictionary-zh-deepseek-pilot-100` 当前为 manual QA findings added。2026-06-22 17:42 JST 完成 scaffold checkpoint；2026-06-22 19:14 JST 用户批准后第一次 Top 100 provider attempt 因 message content 不是 strict JSON 停止；2026-06-22 19:22 JST 已加固 strict JSON：provider schema 统一为 top-level `items`，保留 `response_format: { type: "json_object" }`，prompt 明确禁止 Markdown/code block/解释文字，新增本地 fixture 自测；2026-06-22 20:51 JST 用户批准后仅通过 `node scripts/dictionary/jmdict-zh-deepseek-pilot.js --run-provider` retry 一次，仍因 `DeepSeek message content was not strict JSON.` 停止。没有接受 malformed JSON；已写安全失败 ledger `docs/review/jmdict-zh-deepseek-pilot-100-usage-ledger.json`，实际 tokens/cost unknown。2026-06-22 22:59 JST 未调用 DeepSeek，只加入 `thinking: { type: "disabled" }`、safe last-failure debug 文件逻辑、empty/truncated/reasoning-content fixture 覆盖，以及 `--probe-provider --probe-limit 1` / `--probe-provider --probe-limit 5`。2026-06-22 23:15 JST 用户只批准 1 条 probe 后成功；2026-06-22 23:24 JST 用户只批准 5 条 probe 后成功；2026-06-22 23:35 JST 未调用 DeepSeek，仅修正 prompt/schema/docs：普通日语/EJU 学习者优先，常用义项 `shouldDisplay=true`；麻将、医学、法律、佛教、古语、方言、罕见读法、专门术语默认 `shouldDisplay=false`，并使用 `specialized` / `too_rare` / `archaic` / `dialect` / `needs_human_review` 等标签；`shouldDisplay` 表示默认展示给普通学习者，不表示义项存在；`specialized` 已加入 allowed `issueFlags`。2026-06-22 23:52 JST 用户批准后只执行一次 Top 100 DeepSeek provider run：`node scripts/dictionary/jmdict-zh-deepseek-pilot.js --run-provider`，生成 `docs/review/jmdict-zh-deepseek-pilot-100-review.md` 和 `docs/review/jmdict-zh-deepseek-pilot-100-usage-ledger.json`；generated entries `100`、senses `209`、request count `5`、actual input tokens `30544`、actual output tokens `27411`、providerRunStatus `succeeded`、failedRequestCount `0`、cost null/not configured locally；TextEdit open command 已执行。2026-06-23 00:04 JST 用户人工审核认为整体质量通过但不能直接激活 overlay，本轮未调用任何 provider，仅新增 `docs/review/jmdict-zh-deepseek-pilot-100-qa-findings.md`，记录 `物`、`言う`、`小さい`、`終わる` 和 私/わし/し/わたくし group 的人工 QA。结论：DeepSeek Top 100 明显优于 Google MT baseline，不建议重跑 Top 100，不建议直接激活 overlay，下一步应生成人工修正版 review 或 overlay candidate。`.env.local` ignored/untracked 且未提交。没有调用 Google Translate，没有 runtime AI call，没有 R2/D1 write，没有 deploy，没有 overlay activation，没有 merge/ready。任何 overlay activation / R2/D1 write / Production deploy / PR ready / merge 都需另行明确批准。DeepSeek billing 以控制台为准。
