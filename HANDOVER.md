...
- 中文释义 overlay pilot：Issue #9 / branch `feat/dictionary-zh-overlay-pilot-100` 是 Top 100 pilot，不是全量中文翻译。当前只生成了 `docs/dictionary/zh-overlay-pilot-100/translation-input.json` 和 provider adapter skeleton；必含词全覆盖，估计待翻译英文字符 `7,382`。由于没有 dedicated machine translation provider 配置，translated entries 为 `0`，未调用 provider，未写 R2/D1，未改 Production。下一步需要用户明确选择并批准 `BAINA_ZH_MT_PROVIDER` 和对应 API key；禁止使用 runtime AI 翻译或 LLM teacher explanation 生成词条。
