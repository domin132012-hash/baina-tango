...
- 登录后主界面五栏导航：Issue #11 的 UI 专用分支是 `feat/post-login-nav-restructure`，从 `main` 的 `ebc320317e6ef212a38a53a603191c419aca527c` 创建，不能在词典 overlay 分支 `feat/dictionary-zh-deepseek-pilot-100` 上继续。Draft PR #13：`https://github.com/domin132012-hash/baina-tango/pull/13`，保持 draft/open/unmerged。实现 commit `1f0759015a701c38c20f0bca8a38e02870b07abd` 将底部导航改为 `学习 / 词库 / 首页 / 社区 / 我的`；后续 EJU 深层 bugfix 从 `7d995556dcfa27ebdb61953235de0133a464f418` 开始，提交主题 `Fix EJU deep links in post-login navigation`：`読解` 在 `localhost:4173` 本地静态预览中显示 `需要后端` 并避免 `/api/eju-reading-sets` raw 404 HTML，`記述` 按需加载 `assets/eju-essay.js` 并打开作文 home，综合科目 2024-1 扫描卷可打开题图。验证日志：`docs/review/post-login-nav-restructure-eju-deep-link-validation-log.md`。此任务不得部署 Preview/Production、不得调用 DeepSeek/Google Translate/外部 AI、不得写 R2/D1、不得修改 Cloudflare 配置、不得触碰 PR #12 词典 overlay artifacts。
