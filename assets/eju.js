...
var ejuScanManifest = null; // 扫描卷轻量目录：入口/列表阶段使用
...
var ejuScanBrowserPage = 1;  // 当前扫描浏览页号（PDF page）
...
var EJU_SCAN_BROWSER_PROTOTYPES = {
  ...
  'humanities/2019-1': { title: '総合科目 · 2019年第1回', imageBase: './assets/eju-media/humanities/2019-1/page-' },
  'humanities/2020-2': { title: '総合科目 · 2020年第2回', imageBase: './assets/eju-media/humanities/2020-2/page-' },
  'humanities/2021-1': { title: '総合科目 · 2021年第1回', imageBase: './assets/eju-media/humanities/2021-1/page-' },
  'humanities/2021-2': { title: '総合科目 · 2021年第2回', imageBase: './assets/eju-media/humanities/2021-2/page-' },
  'humanities/2022-1': { title: '総合科目 · 2022年第1回', imageBase: './assets/eju-media/humanities/2022-1/page-' },
  'humanities/2022-2': { title: '総合科目 · 2022年第2回', imageBase: './assets/eju-media/humanities/2022-2/page-' },
  'humanities/2023-1': { title: '総合科目 · 2023年第1回', imageBase: './assets/eju-media/humanities/2023-1/page-' },
  'humanities/2023-2': { title: '総合科目 · 2023年第2回', imageBase: './assets/eju-media/humanities/2023-2/page-' },
...
function ejuScanBrowserProtoFor(key) {
  return (typeof EJU_SCAN_BROWSER_PROTOTYPES !== 'undefined') ? EJU_SCAN_BROWSER_PROTOTYPES[key] : null;
}
...
function ejuHasPracticePrototype(subject, setId) {
  ...
  if (typeof EJU_RIKA_PROTOTYPES !== 'undefined' && EJU_RIKA_PROTOTYPES[key]) return true;
  if (typeof EJU_SOGO_PROTOTYPES !== 'undefined' && EJU_SOGO_PROTOTYPES[key]) return true;
  if (ejuScanBrowserProtoFor(key)) return true;
  return false;
}
...
async function ejuLoadScanManifest() {
  if (ejuScanManifest) return ejuScanManifest;
  if (!ejuScanManifestPromise) {
    ejuScanManifestPromise = (async function() {
      try {
        var res = await fetch('./assets/eju-scan-manifest.json?v=' + EJU_SCAN_DATA_VERSION, { cache: 'force-cache' });
        if (!res.ok) throw new Error('HTTP ' + res.status);
        ejuScanManifest = await res.json();
        return ejuScanManifest;
      } catch(e) {
        ejuScanManifestPromise = null;
        return null;
      }
    })();
  }
  return ejuScanManifestPromise;
}

function ejuHasScanSubject(subject, data) {
  data = data || ejuScanManifest || ejuScannedData;
  return !!(data && (data.sets || []).some(function(s) { return s.subject === subject; }));
}

function ejuScanSubjectInfo(subject, data) {
  data = data || ejuScanManifest || ejuScannedData || {};
  return (data.subjects && data.subjects[subject]) || { label: subject, labelJa: subject };
}

function ejuScanSubjectLabel(subject, data) {
  var info = ejuScanSubjectInfo(subject, data);
...
function ejuImmediateHubCategories(manifest, loading) {
  var cats = ejuFallbackCategories().map(function(cat) { return Object.assign({}, cat); });
  var byId = {};
  cats.forEach(function(cat) { byId[cat.id] = cat; });
  ['sogo', 'science', 'math'].forEach(function(id) {
    if (!byId[id]) return;
    byId[id].available = true;
    byId[id].localScan = true;
    byId[id].scanLoading = !!loading;
  });
  if (manifest) {
    byId.sogo.available = ejuHasScanSubject('humanities', manifest);
    byId.sogo.scanLoading = false;
    byId.science.available = ejuHasScanSubject('science', manifest);
    byId.science.scanLoading = false;
    byId.math.available = ejuHasScanSubject('math1', manifest) || ejuHasScanSubject('math2', manifest);
    byId.math.scanLoading = false;
  }
  return cats;
}

function ejuRenderHubCategories(mount, cats) {
  ...
  (cats || []).forEach(function(cat) {
    ...
    if (cat.scanLoading) html += '<span class="pill" style="margin-top:8px;font-size:11px">目录加载中</span>';
    else if (cat.localScan) html += '<span class="pill ok" style="margin-top:8px;font-size:11px">扫描数据</span>';
...
async function initEjuHub() {
  ...
  if (!mount) return;
  ejuRenderHubCategories(mount, ejuImmediateHubCategories(ejuScanManifest, !ejuScanManifest));
  ejuLoadScanManifest().then(function(manifest) {
    if (!document.body.contains(mount)) return;
    ejuRenderHubCategories(mount, ejuImmediateHubCategories(manifest, false));
  });
}
...
async function renderEjuMathScannedMenu() {
  ...
  var data = await ejuLoadScanManifest();
...
async function renderEjuScannedSubject(subject) {
  ...
  if (!ejuIsReadingSelectRenderCurrent(renderToken)) return;
  if (!data) {
    mount.innerHTML = '<p style="color:#f05b7b">未找到扫描卷目录，请稍后重试。</p>';
    return;
  }
  if (title) title.textContent = ejuScanSubjectLabel(subject, data) + ' — 扫描卷';
  ...
  sets.sort(function(a, b) {
    ...
    var isReady = s.hasGradedPrototype || s.hasScanBrowser || ejuHasPracticePrototype(subject, s.setId);
    var scanOnly = (s.hasScanBrowser || ejuIsScanBrowserPrototype(subject, s.setId))
      && !(s.hasGradedPrototype || ejuHasGradedPracticePrototype(subject, s.setId));
...
function ejuScanPageKindLabel(page) {
  if (!page) return '页面';
  if (page.containsAnswerKey || page.pageType === 'answer_key') return '答案页';
  if (page.pageType === 'instructions') return '说明页';
  if (page.containsQuestions || page.pageType === 'question') return '题目页';
  return page.pageType || '页面';
}

function ejuScanBrowserImageSrc(proto, pageNo) {
  return proto.imageBase + String(pageNo).padStart(3, '0') + '.png';
}

function ejuScanBrowserGo(pageNo) {
  var data = ejuScannedData;
  var item = data && (data.sets || []).find(function(s) {
    return s.subject === ejuCurrentScanSubject && s.setId === ejuCurrentScanSetId;
  });
  if (!item) return;
  ejuScanBrowserPage = pageNo;
  renderEjuScanBrowser(ejuCurrentScanSubject, ejuCurrentScanSetId, item);
}

function renderEjuScanBrowser(subject, setId, item) {
  var key = subject + '/' + setId;
  var proto = ejuScanBrowserProtoFor(key);
  var el = document.getElementById('view-eju-reading-list');
  var mount = el ? el.querySelector('#ejuReadingListMount') : null;
  var titleEl = el ? el.querySelector('#ejuListTitle') : null;
  if (!proto || !mount) return;

  var pages = (item.pages || []).slice().sort(function(a, b) { return Number(a.page || 0) - Number(b.page || 0); });
  if (!pages.length) {
    mount.innerHTML = '<p style="color:#f05b7b">该套卷没有可浏览页面。</p>';
    return;
  }
  if (!pages.some(function(p) { return Number(p.page) === Number(ejuScanBrowserPage); })) {
    ejuScanBrowserPage = Number(pages[0].page);
  }
  var pageIndex = Math.max(0, pages.findIndex(function(p) { return Number(p.page) === Number(ejuScanBrowserPage); }));
  var current = pages[pageIndex] || pages[0];
  var prev = pages[pageIndex - 1];
  var next = pages[pageIndex + 1];
  var imageSrc = ejuScanBrowserImageSrc(proto, current.page);
  var answerCount = pages.filter(function(p) { return p.containsAnswerKey || p.pageType === 'answer_key'; }).length;
  var externalAnswerCount = (item.answerPages || []).length;
  var flagMessages = [];
  if (current.needsReview) {
    if (current.reviewReason) flagMessages.push(current.reviewReason);
    (current.flags || []).forEach(function(flag) {
      if (flag.message) flagMessages.push(flag.message);
      else if (flag.code) flagMessages.push(flag.code);
    });
  }
  ...
  html += '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">'
    ...
    + '<span class="pill" style="font-size:11px">扫描浏览</span>'
    + '<span class="pill" style="font-size:11px">' + ejuEsc(pages.length) + ' 页</span>'
    + '<span class="pill" style="font-size:11px">答案页 ' + ejuEsc(answerCount) + '</span>'
    + (externalAnswerCount ? '<span class="pill" style="font-size:11px">外部答案OCR ' + ejuEsc(externalAnswerCount) + '</span>' : '')
    + '</div>';
  html += '<div style="display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap">'
    + '<button class="ghost" style="padding:8px 12px" ' + (prev ? 'onclick="ejuScanBrowserGo(' + Number(prev.page) + ')"' : 'disabled') + '>上一页</button>'
    + '<span style="font-size:13px;color:#756c9d;font-weight:800">PDF p' + ejuEsc(current.page) + ' · ' + ejuEsc(ejuScanPageKindLabel(current)) + ' · ' + (pageIndex + 1) + '/' + pages.length + '</span>'
    + '<button class="ghost" style="padding:8px 12px" ' + (next ? 'onclick="ejuScanBrowserGo(' + Number(next.page) + ')"' : 'disabled') + '>下一页</button>'
    + '</div>';
  html += '<div style="display:flex;gap:6px;flex-wrap:wrap;max-height:92px;overflow:auto;padding:4px 0">';
  pages.forEach(function(page, idx) {
    var active = Number(page.page) === Number(current.page);
    var label = page.containsAnswerKey || page.pageType === 'answer_key' ? '答' : String(idx + 1);
    html += '<button class="' + (active ? 'primary' : 'ghost') + '" style="padding:6px 9px;font-size:12px;min-width:34px" onclick="ejuScanBrowserGo(' + Number(page.page) + ')" title="PDF p' + ejuEsc(page.page) + '">'
      + ejuEsc(label)
      + '</button>';
  });
  html += '</div>';
  if (flagMessages.length) {
    html += '<div style="padding:10px 12px;border-radius:12px;background:rgba(240,91,123,.08);color:#9b3350;font-size:13px;line-height:1.6">'
      + '<strong>需复核：</strong>' + ejuEsc(flagMessages.join(' / '))
      + '</div>';
  ...
  html += '<details style="border:1px solid rgba(116,106,157,.18);border-radius:14px;background:#fff;padding:12px">'
    ...
    + '<pre style="white-space:pre-wrap;word-break:break-word;color:#4d4770;font-size:12px;line-height:1.55;margin:12px 0 0;max-height:260px;overflow:auto">'
    + ejuEsc((current.text || current.figuresText || '该页暂无 OCR 文本。').slice(0, 6000))
    + '</pre>'
    + '</details>';
  html += '<div style="display:flex;gap:10px;flex-wrap:wrap">'
    + '<button class="ghost" style="flex:1" onclick="renderEjuScannedSubject(&#39;' + ejuJsString(subject) + '&#39;)">返回套卷列表</button>'
    + '</div>';
  html += '</div>';
  mount.innerHTML = html;
