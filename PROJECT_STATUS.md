...
| 中文释义 overlay pilot Top 100 | ⚠️ Draft PR #10 / provider blocked | Issue #9 / draft PR #10 / branch `feat/dictionary-zh-overlay-pilot-100` 已生成 100-entry translation input batch，必含词全覆盖，估计待翻译英文字符 `7,382`。Preview `https://16357ba6.baina-tango.pages.dev` 已确认现有 English lookup 仍为 `dictionarySource=r2-shard`、`食べられる` count `1`、required terms 全部 `aiCalled=false`，但当前未找到 dedicated machine translation provider 配置，因此未翻译、未调用 provider、未写 R2/D1、未改 Production；translated entries `0`。需要用户选择并批准 `BAINA_ZH_MT_PROVIDER` 与对应 API key 后才能生成 machine-translated/unreviewed overlay。 |
