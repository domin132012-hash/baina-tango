...
| DeepSeek 中文释义 overlay pilot | 📝 Draft PR / prompt-schema rule fixed | Issue #11 / PR #12 / branch `feat/dictionary-zh-deepseek-pilot-100` 已有离线 DeepSeek estimator、prompt、design note、review placeholder、guardrails 和失败 ledger。Top 100 provider run 已连续两次因 message content 不是 strict JSON 停止；没有接受 malformed JSON。2026-06-22 22:59 JST 加入 non-thinking request body、last-failure safe debug 文件逻辑、strict JSON fixture 扩展，以及 1/5 条最小探针模式。2026-06-22 23:15 JST 1 条 probe 成功；2026-06-22 23:24 JST 用户只批准 5 条 probe 后，DeepSeek API 仅通过 `node scripts/dictionary/jmdict-zh-deepseek-pilot.js --probe-provider --probe-limit 5` 调用 1 次并成功；generated entries `5`，senses `10`。2026-06-22 23:35 JST 未调用 DeepSeek，仅修正 prompt/schema/docs：普通日语/EJU 学习者优先，`shouldDisplay` 表示默认展示给普通学习者而非义项存在性；麻将、医学、法律、佛教、古语、方言、罕见读法、专门术语默认 `shouldDisplay=false`，并加入合适 issue flags；`specialized` 已加入 allowed `issueFlags`。本轮 Google Translate no; runtime AI calls: `0`; R2/D1 writes: `0`; Production deploy: no; overlay activation: no; `.env.local` ignored/untracked and not committed. 下一步必须另行批准任何 Top 100 DeepSeek run。剩余费用风险：前两次失败请求加 1 条/5 条成功 probe 可能已有费用，最终以 DeepSeek 控制台为准。 |
