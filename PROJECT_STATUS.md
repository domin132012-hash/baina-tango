...
| DeepSeek 中文释义 overlay pilot Top 100 | 🛠️ Draft PR / strict JSON hardened, retry not approved | Issue #11 / PR #12 / branch `feat/dictionary-zh-deepseek-pilot-100` 已有离线 DeepSeek Top 100 estimator、prompt、design note、review placeholder 和 guardrails。2026-06-22 19:14 JST 用户批准后仅通过 `node scripts/dictionary/jmdict-zh-deepseek-pilot.js --run-provider` 调用 DeepSeek 一次；脚本因 provider message content 不是 strict JSON 而停止，没有接受 malformed JSON，没有生成 AI review artifact，没有写 usage ledger。2026-06-22 19:22 JST 已加固：provider schema 统一为 top-level `items`，保留 `response_format: { type: "json_object" }`，prompt 明确禁止 Markdown/code block/解释文字，新增本地 fixture 自测 `11/11`。本轮 DeepSeek API called: no; Google Translate called again: no; runtime AI calls: `0`; R2/D1 writes: `0`; Production deploy: no; overlay activation: no; `.env.local` ignored/untracked and not committed. 下一次 provider retry 仍需用户单独批准。 |
