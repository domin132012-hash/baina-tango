...
> ⚠️ EJU 記述作文 PR #2 仍是 draft，未合并；未登录链路已验收，登录后真实批改/追问仍缺已确认账号验收。
...
| EJU 記述作文双知识库 | 🛠️ Draft PR #2，未合并 | `feat/eju-essay-integration` 已把批改拆成 `rubric`（基礎編评分依据）+ `reference bank`（実践編例子/表达）；`renderEjuJapanese()` 中的 `記述` 入口已直接打开并显示 `试验开放`。2026-06-16 验收：入口与未登录 401 通过，已补请求体/题目/作文/追问/上下文长度限制和错误脱敏；因没有可用已确认邮箱账号，登录后 analyze/follow-up 未真实通过，不能 ready/merge |
