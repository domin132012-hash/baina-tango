...
| prompt | `scripts/dictionary/prompts/jmdict-zh-deepseek-system.md`（含 editorial rules） |
| batch size | 20 entries/request |
| estimated requests | 25 |
...
## Updated Guardrails Required

⚠️ 当前 guardrail（为 Top 500 配置）不适用于 Top 1000。必须先更新环境变量：

```bash
export BAINA_ZH_AI_APPROVE_RUN=YES_DEEPSEEK_TOP_1000_ONLY
export BAINA_ZH_AI_MAX_ENTRIES=500
export BAINA_ZH_AI_MAX_INPUT_TOKENS=92459
export BAINA_ZH_AI_MAX_OUTPUT_TOKENS=123074
export BAINA_ZH_AI_MAX_TOTAL_ESTIMATED_TOKENS=221127
...
- [ ] **我确认范围仅 Top 1000（JMdict entries 500–999）**
